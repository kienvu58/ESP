package com.nhom1.esp.esppart2b;
...
public class RMActivity extends AppCompatActivity {
    ...
    ArrayList<EditText> TList = new ArrayList<>();
    TextView timeTV;
    ...
    private void getView() {
        ...
        for (int i = 0; i < 9; i++) {
            String name = "T" + i;
            int id = getResources().getIdentifier(name, "id", getPackageName());
            TList.add((EditText) findViewById(id));
        }
    }
    ...
    private void computeSchedule() {
        ...
        isSchedulable123 = rmScheduleJNI(id, C, T, 3, schedule123, nSchedules);

        id = new int[]{4, 5};
        getTaskInformation(id, 2);
        isSchedulable45 = rmScheduleJNI(id, C, T, 2, schedule45, nSchedules);

        id = new int[]{6, 7, 8, 9};
        getTaskInformation(id, 4);
        isSchedulable6789 = rmScheduleJNI(id, C, T, 4, schedule6789, nSchedules);
    }

    private void getTaskInformation(int[] id, int nIds) {
        for (int i = 0; i < nIds; i++) {
            C[i] = Integer.valueOf(CList.get(id[i] - 1).getText().toString());
            T[i] = Integer.valueOf(TList.get(id[i] - 1).getText().toString());
        }
    }
    ...
    private void initializeTaskInformation() throws IOException {
        try {
            ...
            while ((line = br.readLine()) != null && i < 9) {
                ...
                TList.get(i).setText(String.valueOf(a[2]));
                i++;
            }
    ...
    /* native method */
    public native int rmScheduleJNI(int id[], int C[], int T[], int nTasks, int result[], int nResults[]);
